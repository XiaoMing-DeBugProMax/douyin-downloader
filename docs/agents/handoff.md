...
3. Read the approved parent specification and live tickets, including comments
   and blocking edges:
...
1. `AGENTS.md` for non-negotiable project, security, privacy, test, and build
   rules.
2. GitHub Issue #1 and its child ticket bodies, comments, state, and blocking
   edges for approved scope and live progress. GitHub Issues are the canonical
   tracker; see `docs/agents/issue-tracker.md`.
3. `docs/superpowers/specs/2026-07-30-douyin-local-archive-foundation.md` for
   the current archive-foundation cycle, while
...
Use `/ask-matt` when the correct flow is uncertain. For this project, apply the
following routes:
...
6. Use `/code-review` against a fixed base on both axes: Standards and Spec.
   ...
   verification.
7. Commit the completed work on its feature branch. Ask the user which supported
   integration path to take before merging, pushing, creating a PR, or closing
   an issue unless that authority was already explicit.
...
## Stable checkpoint as of 2026-08-10

- Local and remote `main` were synchronized at `78d4b05`.
- Cycle 1 tickets #2 through #6 were integrated into `main`, pushed, had all
  acceptance boxes checked, and were closed as completed.
- Parent specification Issue #1 remains open.
- Tickets #7 through #14 remain open. The next intended sequential ticket is #7
  (persistent task center and three-level progress), but the next conversation
  must re-read all open ticket blockers because more than one dependency frontier
  may be available.
- The last integrated verification reported 254 passed and 1 skipped, with Ruff
  and strict mypy passing. Issue #6 also passed the Windows PyInstaller build,
  sensitive-data scans, FFmpeg distribution audit, and packaged-resource checks.
- No feature worktree or feature branch was intentionally left active at this
  checkpoint.
...
5. Do not push, close issues, create PRs, or alter parent issues without explicit
   user authority. Closing a completed child ticket does not imply closing Issue
   #1.
...
> Issues and Git. Use the Matt Pocock workflow. Select only an approved frontier
> ticket whose blockers are closed, and do not push, close issues, or create a PR
